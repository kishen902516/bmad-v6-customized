...
<action>Copy .gitignore template from {project-root}/bmad/spring-boot-clean-arch/templates/git/gitignore.template</action>

<action>Copy commit message template from {project-root}/bmad/spring-boot-clean-arch/templates/git/commit-template.txt to .gitmessage</action>
</step>

<step n="11" goal="Initialize Git and Git Flow">
<ask>Initialize Git repository and Git Flow?
1. Yes - Full setup (git, Git Flow, GitHub integration)
2. Git only - Local repository without GitHub
3. Skip - No version control

Choose [1/2/3]:</ask>

<action>Store git_setup_choice</action>

<check if="git_setup_choice != 'skip'">
  <action>Initialize git repository in project directory:
  - cd {default_output_path}
  - git init
  - Configure commit template: git config commit.template .gitmessage
  </action>

  <action>Create initial commit structure with all generated files</action>

  <check if="git_setup_choice == 'yes'">
    <ask>Do you want to create a GitHub repository?
    1. Yes - Create new GitHub repository
    2. No - Use existing repository
    3. Skip - Local only for now

    Choose [1/2/3]:</ask>

    <action>Store github_repo_choice</action>

    <check if="github_repo_choice == 'yes'">
      ...
      <action>Store repo_visibility</action>

      <action>Create GitHub repository using gh CLI:
      - gh repo create {project_name} --{repo_visibility} --source=. --remote=origin
      - Set repository description: {project_description}
      </action>

      <action>Display: ✅ GitHub repository created at https://github.com/{username}/{project_name}</action>
    </check>

    <check if="github_repo_choice == 'existing'">
      <ask>Enter GitHub repository URL (e.g., https://github.com/user/repo.git):</ask>

      <action>Store repo_url</action>

      <action>Add remote: git remote add origin {repo_url}</action>
    </check>

    <action>Setup Git Flow (main + develop branches):
    - Copy and run gitflow-init.sh template
    - Create main branch
    - Create develop branch
    - Set develop as default branch for development
    </action>

    <action>Display Git Flow structure:
    📋 Git Flow Branches:
    ...
    <check if="github_repo_choice != 'skip'">
      <ask>Create GitHub Project board for issue tracking?
      [y/n]:</ask>

      <check if="yes">
        <action>Create GitHub Project (Projects V2):
        - gh project create --owner {username} --title "{project_name} - Development"
        - Add custom fields: TDD Phase, Layer, Component Type, Estimated Hours
        - Create views: Board (by status), Table (all fields), TDD Progress (by TDD phase)
        </action>

        <action>Link project to repository</action>

        <action>Display: ✅ GitHub Project created and linked to repository</action>
      </check>
    </check>
  </check>
  ...
  - Add features using BMAD workflows"
  </action>

  <check if="github_repo_choice != 'skip' && github_repo_choice != 'local'">
    <action>Push initial commit:
    - git push -u origin main
    - git push -u origin develop
    </action>

    <action>Display: ✅ Initial commit pushed to GitHub</action>
  </check>

  <action>Switch to develop branch for future development:
  - git checkout develop
  </action>

  <action>Display:
  ✅ Git and Git Flow initialized successfully!
  📍 Current branch: develop (ready for feature development)
  </action>
</check>
</step>

<step n="12" goal="Create TDD workflow documentation">
<action>Copy TDD Workflow Guide from {project-root}/bmad/spring-boot-clean-arch/data/TDD-WORKFLOW-GUIDE.md to project docs/</action>

<action>Create CONTRIBUTING.md with:
- Git Flow workflow explanation
- Feature branch naming: feature/{issue-number}-{description}
- TDD requirement: All features follow Red-Green-Refactor
...
📖 Development Guidelines Created:
  - docs/TDD-WORKFLOW-GUIDE.md: Complete TDD workflow guide
...
<step n="13" goal="Validate generated project">
...
<check if="tests fail">
  ...
  <goto step="13">Re-validate after fixes</goto>
</check>
...
- Git initialized: {git_setup_choice}
- GitHub repository: {github_repo_url (if applicable)}
- GitHub Project: {github_project_url (if applicable)}
...
4. Read docs/TDD-WORKFLOW-GUIDE.md for TDD workflow
5. Use *add-entity to add new domain entities (follows TDD)
6. Use *add-use-case to add new use cases (follows TDD)
7. Use *add-rest-endpoint to add new API endpoints (follows TDD)

💡 All features follow Test-Driven Development (Red-Green-Refactor)
📋 Track features in GitHub Project: {github_project_url}
